...
import android.xml.plist.PListXMLConstants;
...
public class PList {
	...
	private void attachPListObjToDictParent(PListObject obj,
		...
		Log.v(PListXMLStringer.newBuilder().append(TAG)
				.append("#attachPListObjToDictParent").toString(),
				PListXMLStringer.newBuilder().append("key|obj-type|obj: ").append(key)
						.append(PListXMLConstants.PIPE).append(obj.getType())
						.append(PListXMLConstants.PIPE).append(obj.toString())
	...
	private void attachPListObjToArrayParent(Stack<PListObject> stack,
		...
		Log.v(PListXMLStringer.newBuilder().append(TAG)
				...
				PListXMLStringer.newBuilder().append("obj-type|obj: ")
						.append(PListXMLConstants.PIPE).append(obj.getType())
						.append(PListXMLConstants.PIPE).append(obj.toString())
						.append(PListXMLConstants.PIPE).toString());
	 ...
	 * the tags defined in {@link PListXMLConstants}.
	 * 
	 * @param tag
	 * @param value
	 *            can be null if tag equals {@link PListXMLConstants#TAG_BOOL_FALSE} or
	 *            {@link PListXMLConstants#TAG_BOOL_TRUE}.
	 * @throws Exception
	 * 
	...
	public PListObject buildObject(java.lang.String tag, java.lang.String value)
		...
		if (tag.equalsIgnoreCase(PListXMLConstants.TAG_INTEGER)) {
			ret = new Integer();
			((Integer) ret).setValue(value);
		} else if (tag.equalsIgnoreCase(PListXMLConstants.TAG_STRING)) {
			ret = new String();
			((String) ret).setValue(value);
		} else if (tag.equalsIgnoreCase(PListXMLConstants.TAG_REAL)) {
			ret = new Real();
			((Real) ret).setValue(value);
		} else if (tag.equalsIgnoreCase(PListXMLConstants.TAG_DATE)) {
			ret = new Date();
			((Date) ret).setValue(value);
		} else if (tag.equalsIgnoreCase(PListXMLConstants.TAG_BOOL_FALSE)) {
			ret = new False();
		} else if (tag.equalsIgnoreCase(PListXMLConstants.TAG_BOOL_TRUE)) {
			ret = new True();
		} else if (tag.equalsIgnoreCase(PListXMLConstants.TAG_DATA)) {
			ret = new Data();
			((Data) ret).setValue(value.trim(), true);
		} else if (tag.equalsIgnoreCase(PListXMLConstants.TAG_DICT)) {
			ret = new Dict();
		} else if (tag.equalsIgnoreCase(PListXMLConstants.TAG_PLIST_ARRAY)) {
			ret = new Array();
		}
		return ret;
	}
