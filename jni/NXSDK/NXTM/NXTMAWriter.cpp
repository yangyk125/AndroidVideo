...
#include "NXTMAWriter.h"

NXTMAWriter::NXTMAWriter()
{
...
bool NXTMAWriter::open(const char* filepath, const char* suffix, NXIN NXSDKMediaFormat& mediaFormat)
{
	_filepath = filepath ;
	...
	if(!_fileObjt->open("wb")) {
		__android_log_print(ANDROID_LOG_ERROR, "NXTM", "TMP4 Async Writer Open File Failure!!");
		return false ;
	}
	...
	if(!_x264Encoder) {
		__android_log_print(ANDROID_LOG_ERROR, "NXTM", "TMP4 Async Writer Get X264 Encoder Failure!!");
		return false ;
	}
	...
	if(_avscaler == NULL) {
		__android_log_print(ANDROID_LOG_ERROR, "NXTM", "TMP4 Async Writer sws_getContext() Failure!!");
		return false ;
	}
...
bool NXTMAWriter::append_buffer(NXPtr<NXBuffer>& buffer,NXIN NXSDKTrackType track)
{
	if(_wrtThread == NULL)
		return false ;
...
void NXTMAWriter::on_thread_runing(const NXThread* thread, NXPtr<NXObject>& param)
{
	while(_stsThread == NXThreadRunning)
	{
		...
		{
			_wrtLocker.lock() ;
			TMBufferItem bufferItem = _wrtDequer.front() ;
			_wrtDequer.pop_front() ;
			_wrtLocker.unlock() ;

			if(!encode_buffer(bufferItem.buffer, bufferItem.track)) {
				__android_log_print(ANDROID_LOG_ERROR, "NXTM", "TMP4 Async Writer write_buffer() Failure !!");
				return ;
			}
		}
	...
	__android_log_print(ANDROID_LOG_DEBUG, "NXTM", "TMP4 Async Writer Thread Exit Success !!");
}

bool NXTMAWriter::encode_buffer(NXPtr<NXBuffer>& buffer,NXIN NXSDKTrackType track)
{
	if(track == NXSDKTrackVideo)
	{
		...
		__android_log_print(ANDROID_LOG_DEBUG, "NXTM", "TMP4 Async Writer Scale Video cost ticks = %u !!", ticks2-ticks1);
#endif

		if(ret != _nxformat.video_format.height) {
			__android_log_print(ANDROID_LOG_ERROR, "NXTM", "TMP4 Async Writer sws_scale() failure!!");
			return false ;
		}
		...
		if(x264_encoder_encode(_x264Encoder, &sNal, &iNal, _x264ImageI, _x264ImageO) < 0) {
			__android_log_print(ANDROID_LOG_ERROR, "NXTM", "TMP4 Async Writer x264_encoder_encode() failure !!");
			return false;
		}
		...
		if(iNal < 0) {
			...
			__android_log_print(ANDROID_LOG_WARN, "NXTM", "TMP4 Async Writer x264_encoder_encode() got_packet=0 !!");
			...
			__android_log_print(ANDROID_LOG_DEBUG, "NXTM", "TMP4 Async Writer Encode Video cost ticks = %u !!", ticke2-ticke1);
#endif
...
#ifdef DEBUG
		__android_log_print(ANDROID_LOG_DEBUG, "NXTM", "TMP4 Async Writer append_buffer() Video success!!");
#endif
	...
	else if(track == NXSDKTrackAudio1)
	{
		...
		__android_log_print(ANDROID_LOG_ERROR, "NXTM", "TMP4 Async Writer buffer track not supported !!");
		return false ;
	}
	...
	while(_aswtQueue.size() > 0) {
		...
		MPTPacketHeader packet ;

		packet.packet_track  = firstItem.track ;
		packet.packet_flags  = firstItem.flags ;
		packet.packet_length = firstItem.legth ;

		_fileObjt->write((uint8_t*)&packet, sizeof(packet));
...
bool NXTMAWriter::flush_buffers()
{
...
#ifdef DEBUG
		__android_log_print(ANDROID_LOG_DEBUG, "NXTM", "TMP4 Async Writer flush_buffer() Video Success, Last Buffer = %u !!", (uint32_t)_aswtQueue.size());
#endif
	}
...
bool NXTMAWriter::stop()
{
	_stsThread = NXThreadExiting ;
	return true ;
}

bool NXTMAWriter::stoped()
{
	_wrtThread->stoped() ;
	_wrtThread = NULL ;
