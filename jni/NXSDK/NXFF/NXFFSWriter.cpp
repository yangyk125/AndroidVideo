...
bool NXFFSWriter::open(const char* filepath, const char* suffix, NXIN NXSDKMediaFormat& mediaFormat)
{
	_filepath = filepath ;
	...
	if(_outformat == NULL) {
		__android_log_print(ANDROID_LOG_ERROR, "NXFF", "FFMPEG Sync Writer av_guess_format(%s) Failure!!", suffix);
		return false;
	}
	_avcontext = avformat_alloc_context();
	if(_avcontext== NULL) {
		__android_log_print(ANDROID_LOG_ERROR, "NXFF", "FFMPEG Sync Writer avformat_alloc_context() Failure!!");
		return false ;
	}
	int ret = avformat_alloc_output_context2(&_avcontext, _outformat, NULL, _filepath.c_str());
	if (ret < 0) {
		__android_log_print(ANDROID_LOG_ERROR, "NXFF", "FFMPEG Sync Writer avformat_alloc_output_context2() Failure!!");
		return false ;
	}
	...
	if(_nxformat.track_mask & NXSDKTrackVideo) {
		...
		if(_avcodecV == NULL) {
			__android_log_print(ANDROID_LOG_ERROR, "NXFF", "FFMPEG Sync Writer avcodec_find_encoder(H264) Failure!!");
			return false ;
		}
	}
	...
	if(_nxformat.track_mask & NXSDKTrackAudio1) {
		...
		if(_avcodecA == NULL) {
			__android_log_print(ANDROID_LOG_ERROR, "NXFF", "FFMPEG Sync Writer avcodec_find_encoder(AAC) Failure!!");
			return false ;
		}
	}

	if((_nxformat.track_mask & NXSDKTrackVideo) && _avcodecV == NULL) {
		__android_log_print(ANDROID_LOG_ERROR, "NXFF", "FFMPEG Sync Writer V avcodec_find_encoder() Failure!!");
		return false ;
	}
	if((_nxformat.track_mask & NXSDKTrackAudio1) && _avcodecA == NULL) {
		__android_log_print(ANDROID_LOG_ERROR, "NXFF", "FFMPEG Sync Writer A avcodec_find_encoder() Failure!!");
		return false ;
	}
	...
	if(_avstreamV == NULL) {
		__android_log_print(ANDROID_LOG_ERROR, "NXFF", "FFMPEG Sync Writer V avformat_new_stream() Failure!!");
		return false ;
	}
	...
	if (avcodec_open2(_avstreamV->codec, _avcodecV, NULL)) {
		__android_log_print(ANDROID_LOG_ERROR, "NXFF", "FFMPEG Sync Writer V avcodec_open2() Failure!!");
		return false;
	}
	...
	if(_avcodecA)
	{
		...
		if(_avstreamA == NULL) {
			__android_log_print(ANDROID_LOG_ERROR, "NXFF", "FFMPEG Sync Writer A avformat_new_stream() Failure!!");
			return false ;
		}
		...
		if (avcodec_open2(_avstreamA->codec, _avcodecA, NULL) < 0) {
			__android_log_print(ANDROID_LOG_ERROR, "NXFF", "FFMPEG Sync Writer A avcodec_open2() Failure!!");
			return false;
		}
	}
	...
	if(_avscaleV == NULL) {
		__android_log_print(ANDROID_LOG_ERROR, "NXFF", "FFMPEG Sync Writer sws_getContext() Failure!!");
		return false ;
	}

	//打开创建文件
	if (avio_open(&_avcontext->pb, _filepath.c_str(), AVIO_FLAG_WRITE) < 0) {
		__android_log_print(ANDROID_LOG_ERROR, "NXFF", "FFMPEG Sync Writer avio_open() Failure!!");
		return false;
	}

	//写入头信息
	if (avformat_write_header(_avcontext, NULL) < 0) {
		__android_log_print(ANDROID_LOG_ERROR, "NXFF", "FFMPEG Sync Writer avformat_write_header() Failure!!");
		return false;
	}

	_avframeV = av_frame_alloc();
	if (_avframeV == NULL) {
		__android_log_print(ANDROID_LOG_ERROR, "NXFF", "FFMPEG Sync Writer av_frame_alloc() Failure!!");
		return false;
	}
	...
	if (_audififo == NULL) {
		__android_log_print(ANDROID_LOG_ERROR, "NXFF", "FFMPEG Sync Writer av_audio_fifo_alloc() Failure!!");
		return false;
	}
...
bool NXFFSWriter::start()
{
	return true ;
}
bool NXFFSWriter::append_buffer(NXPtr<NXBuffer>& buffer,NXIN NXSDKTrackType trackType)
{
	if(trackType == NXSDKTrackVideo)
	{
		...
		if(ret != _nxformat.video_format.height) {
			__android_log_print(ANDROID_LOG_ERROR, "NXFF", "FFMPEG Sync Writer sws_scale() failure!!");
			return false ;
		}
		...
		if(ret < 0) {
			__android_log_print(ANDROID_LOG_ERROR, "NXFF", "FFMPEG Sync Writer avcodec_encode_video2() failure!!");
			return false;
		}
		if(!got_packet) {
			__android_log_print(ANDROID_LOG_WARN, "NXFF", "FFMPEG Sync Writer avcodec_encode_video2() got_packet=0 !!");
			return true;
		}

#ifdef PRINT_ENCODE_VIDEO_TIME
		uint32_t tick2 = NXKit::get_time_tick32() ;
		__android_log_print(ANDROID_LOG_DEBUG, "NXFF", "FFMPEG Sync Writer encode Video cost ticks = %u !!", tick2-tick1);
#endif
		...
		if(ret < 0)
		{
			__android_log_print(ANDROID_LOG_ERROR, "NXFF", "FFMPEG Sync Writer av_write_frame() failure, v packet size = %d!!", _vpacket.size);
			return false;
		}

#ifdef DEBUG
		__android_log_print(ANDROID_LOG_DEBUG, "NXFF", "FFMPEG Sync Writer append_buffer() Video success!!");
#endif
	}
	else if(trackType == NXSDKTrackAudio1)
	{
		...
		if(retsmp != allsmp) {
			__android_log_print(ANDROID_LOG_ERROR, "NXFF", "FFMPEG Sync Writer A av_audio_fifo_write() failure!!");
			return false;
		}
		...
		while(av_audio_fifo_size(_audififo) > maxsmps)
		{
			...
			if(maxsmps != av_audio_fifo_read(_audififo, (void**)&pptrtmp, maxsmps)) {
				__android_log_print(ANDROID_LOG_ERROR, "NXFF", "FFMPEG Sync Writer A av_audio_fifo_read() failure!!");
				return false;
			}
			...
			if (_avstreamA->codec->sample_fmt == AV_SAMPLE_FMT_S16) {
				...
				__android_log_print(ANDROID_LOG_ERROR, "NXFF", "FFMPEG Sync Writer A unsupported PCM Format !!!");
				return false ;
			}
		...
		uint32_t tick1 = NXKit::get_time_tick32() ;
			...
			if (retlen < 0) {
				__android_log_print(ANDROID_LOG_ERROR, "NXFF", "FFMPEG Sync Writer avcodec_encode_audio2() failure error = %d", retlen);
				return false;
			}
			if (!got_packet) {
				__android_log_print(ANDROID_LOG_WARN, "NXFF", "FFMPEG Sync Writer avcodec_encode_audio2() got_packet=0 !!");
				return true;
			}

#ifdef PRINT_ENCODE_AUDIO_TIME
		uint32_t tick2 = NXKit::get_time_tick32() ;
		__android_log_print(ANDROID_LOG_DEBUG, "NXFF", "FFMPEG Sync Writer encode Audio cost ticks = %u !!", tick2-tick1);
#endif
			...
			if (ret < 0) {
				__android_log_print(ANDROID_LOG_ERROR, "NXFF", "FFMPEG Sync Writer av_write_frame() failure, a packet size = %d!!", _apacket.size);
				return false;
			}
		}

#ifdef DEBUG
		__android_log_print(ANDROID_LOG_DEBUG, "NXFF", "FFMPEG Sync Writer append_buffer() Audio success!!");
#endif
	}
	return true ;
}

bool NXFFSWriter::audio_trailer()
{
	int smplen  = (_nxformat.audio_format.channels*_nxformat.audio_format.sample_bits/8) ;
...
bool NXFFSWriter::flush_buffer(NXSDKTrackType track)
{
...
#ifdef DEBUG
		__android_log_print(ANDROID_LOG_DEBUG, "NXFF", "FFMPEG Sync Writer Flush Video success!!");
#endif
		return true ;
	}
...
#ifdef DEBUG
		__android_log_print(ANDROID_LOG_DEBUG, "NXFF", "FFMPEG Sync Writer Flush Audio success!!");
#endif
		return true ;
	}
...
bool NXFFSWriter::stoped()
{
	if(av_write_trailer(_avcontext) < 0) {
		__android_log_print(ANDROID_LOG_ERROR, "NXFF", "FFMPEG Sync Writer av_write_trailer() Failure!!");
		return false ;
	}
	...
	if(avformat_flush(_avcontext) < 0) {
		__android_log_print(ANDROID_LOG_ERROR, "NXFF", "FFMPEG Sync Writer avformat_flush() Failure!!");
		return false ;
	}

	if(avio_close(_avcontext->pb) < 0) {
		__android_log_print(ANDROID_LOG_ERROR, "NXFF", "FFMPEG Sync Writer avio_close() Failure!!");
		return false ;
	}
